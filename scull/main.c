...
static void scull_create_proc(void){
	/*struct proc_dir_entry *proc_file_entry;
        proc_file_entry = proc_create("scullseq",0,NULL,&scull_proc_ops);
        if(proc_file_entry)
                return -ENOMEM;*/
...
int scull_init_module(void){
	...
	if(!scull_devices){
		result = -ENOMEM;
		printk(KERN_WARNING "Can not assign devices\n");
		goto fail;
	}
